...
export function TimelinePage() {
  ...
  // Filter tracks scoped to current parentEventId
  const tracks = useMemo(() => {
    const scopedTracks = storedTracks.filter(t =>
      (t.parentEventId || null) === parentEventId
    )
    if (scopedTracks.length === 0) {
      return [{ id: parentEventId ? `main-${parentEventId}` : 'main', name: 'Main Story', color: TRACK_COLORS[0] }]
    }
    return scopedTracks.map(t => ({ id: t.id, name: t.name, color: t.color }))
  }, [storedTracks, parentEventId])
  ...
  const handleAddTrack = useCallback(() => {
    ...
    const scopedTracks = storedTracks.filter(t => (t.parentEventId || null) === parentEventId)
    if (scopedTracks.length === 0) {
      const mainTrack: TimelineTrack = {
        id: parentEventId ? `main-${parentEventId}` : 'main',
        storyId,
        name: 'Main Story',
        order: 0,
    ...
    const newTrackIndex = scopedTracks.length === 0 ? 1 : tracks.length
    ...
    addTimelineEvent(storyId, newEvent)
  }, [storyId, storedTracks, tracks.length, parentEventId, addTimelineTrack, addTimelineEvent])
  ...
  const handleDeleteTrack = useCallback((trackId: string) => {
    if (trackId === 'main' || trackId.startsWith('main-')) return // Don't delete main track
    const trackIndex = tracks.findIndex(t => t.id === trackId)
    if (trackIndex === -1) return

    // Only consider events within the current scope (matching parentEventId)
    const scopedEvents = visibleEvents
    const trackEvents = scopedEvents.filter(e => e.track === trackIndex)
    ...
    setConfirmDialog({
      ...
      message,
      onConfirm: () => {
        // Delete all events on this track (scoped)
        trackEvents.forEach(e => {
          deleteTimelineEvent(storyId, e.id)
        })

        // Shift scoped events on higher tracks down by 1
        scopedEvents.filter(e => e.track > trackIndex).forEach(e => {
  ...
  }, [storyId, tracks, visibleEvents, deleteTimelineTrack, deleteTimelineEvent, updateTimelineEvent])
  ...
  return (
    <div className="h-full flex flex-col bg-slate-900">
      {/* Header */}
      <header className="flex-shrink-0 border-b border-slate-700/80 pl-20 pr-4 py-3 flex items-center justify-between bg-slate-900/95 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-semibold text-slate-200">{story?.title || "Story"} - Timeline</h1>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-200/50">
